...
1. **LLM Team** – APIs to run AI classification, scoring, rubric evaluation, similarity detection, and feedback generation.
2. **Frontend ↔ Backend (Product Engineering)** – APIs for user authentication, submission handling, teacher review, file management, and policy control.
...
Purpose: Upload and evaluate a submission.
...
Purpose: Student submits work (text or file) with optional GenAI chat log; backend calls:
...
- **Prompt ID:** stable ID (e.g., "P001") sent with submissions for evaluation and similarity checks.
- **Versioning:** resubmits keep **same submissionId**, increment **version**.
- **Visibility:**
  - **Student:** feedback + public rubric only.
  - **Teacher/Admin:** + classification, AI score, similarity %, highlights, versions.
- **Feature flag:** enable Future features via future_scoring=true without impacting T2.
- **Security/Privacy:** prefer **redaction** before LLM; enforce RBAC on all teacher-only fields.